...
async function fork (params, shared_callback) {
    ...
    for (var i=0;i<params.length;i++)
        await processFile(params[i], callback);
}
...
function main(){
    var paths = walk.sync(folderToWalk);
    console.log(paths);
    for(var i = 0; i < paths.length; i++){
        var path = paths[i];
        // To skip directories ('file' event in async)
        if(!path.includes('.'))
            continue;
        var fileCount = fs.readFileSync(path).toString('utf8')
            .split(/\r\n|[\n\r\u0085\u2028\u2029]/g)
            .length-1;
        printer.printFileCount(path, fileCount);
        wholeCount += fileCount;
    }
    printer.printSetCount(wholeCount);
}
